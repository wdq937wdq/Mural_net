...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        self.lw = line_width or max(round(sum(im.size if input_is_pil else im.shape) / 2 * 0.002), 2)
        if self.pil:  # use PIL
            ...
            try:
                ...
                size = font_size or max(round(sum(self.im.size) / 2 * 0.020), 10)  # 减小PIL字体
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # 新代码: 实现半透明背景
                alpha = 0.4  # 透明度设置 (0.0 完全透明 - 1.0 完全不透明)，建议 0.3 到 0.5
                try:
                    # 确保坐标在图像范围内，防止越界报错
                    x1, y1 = max(0, p1[0]), max(0, min(p1[1], p2[1]))
                    x2, y2 = min(self.im.shape[1], p2[0]), min(self.im.shape[0], max(p1[1], p2[1]))
